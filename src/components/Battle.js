import React, { Component } from 'react';

export default class Battle extends Component {
  constructor(props){
    super(props);
    this.state = { 
      pokemons: {...props.pokemons},
      poke1HP: props.pokemons[0].stats[0].base_stat,
      poke2HP: props.pokemons[1].stats[0].base_stat,
      battleStatus: ["Start Battle!"]
    }
  }

  render() {
    return <>
      <div style={{width: "100%"}}>
        <h4 style={{display: "flex", justifyContent: "center", alignItems: "center"}}>
          {this.state.pokemons[0].name} vs {this.state.pokemons[1].name}
        </h4>
         {
            this.state.battleStatus.map(message => {
              return (
                <h5>
                  {message}
                </h5>
              )
            })
          }
        <div key={this.state.pokemons[0].name} class="col-sm" 
          style={{
            border: "5px solid black",
            borderRadius: "10px",
            margin: "0 10px 0 10px",
            display: "flex",
            justifyContent: "center",
            alignItems: "center"}}> 
          <div style={{paddingTop: "20px"}}>
            <div style={{
              backgroundImage: 'url('+this.state.pokemons[0].sprites.back_default + ')',
              width: "100px",
              height: "100px"}} 
              alt={this.state.pokemons[0].name+" back_default picture"}>
            </div>
            <div style={{position: "relative"}}>
              <div style={{
                position: "absolute",
                zIndex: "8",
                backgroundColor: "green",
                width: (this.state.poke1HP/this.state.pokemons[0].stats[0].base_stat*100)+"%",
                height: "20px"}}></div>
              <div style={{
                position: "absolute",
                zIndex: "10"}}>
                HP: {this.state.poke1HP}/{this.state.pokemons[0].stats[0].base_stat}
              </div>
            </div>
          </div>
          <div 
            style={{
              paddingTop: "40px",
              paddingBottom: "80px",
              transform: "translate(-20px, 0)"
            }}>
            <div style={{position: "relative"}}>
              <div style={{
                position: "absolute",
                zIndex: "8",
                backgroundColor: "green",
                width: (this.state.poke2HP/this.state.pokemons[1].stats[0].base_stat*100)+"%",
                height: "20px"}}>
              </div>
              <div style={{
                position: "absolute",
                zIndex: "10"
                }}>
                HP: {this.state.poke2HP}/{this.state.pokemons[1].stats[0].base_stat}
              </div>
            </div>
            <div style={{backgroundImage: 'url('+this.state.pokemons[1].sprites.front_default + ')', width: "100px", height: "100px"}} alt={this.state.pokemons[1].name+" front_default picture"}>
            </div>
          </div>
        </div>
        <div style={{display: "flex", justifyContent: "center", alignItems: "center"}}>
          <button onClick={this.onAttack}>Attack</button>
          <button >Defend</button>
          <button >Items</button>
        </div>
      </div>
    </>
  }

  onAttack = () => {
    /** 
     * Hit is calculated using two random numbers: 
     * 1 considering the defense of player 2 
     * 1 considering the attack of player 1
     */
    const randomizeDefense = Math.floor(Math.random()*this.state.pokemons[1].stats[2].base_stat);

    const randomizeAttack = Math.floor(Math.random()*this.state.pokemons[0].stats[1].base_stat);
    const hit = randomizeAttack-randomizeDefense > 0 ? randomizeAttack - randomizeDefense : 0;
    const HP = this.state.poke2HP - hit > 0 ? (this.state.poke2HP - hit) : 0;
    let messages = [this.state.pokemons[1].name + (hit ? ` took ${hit} in damage` : ` successfully blocked hit`)];
    if (HP === 0) {
      messages.push(`${this.state.pokemons[1].name} passed out`);
      messages.push(`${this.state.pokemons[0].name} won`);
      this.sendWinner(this.state.pokemons[0].name);
    }
    this.setState({poke2HP: HP});
    this.setState({battleStatus: messages});
    if (HP !== 0) {
      setTimeout(()=> {
        this.onAttackNPC();
        console.log("Waited 3 seconds");
      }, 3000);
    }
  }

  onAttackNPC = () => {
    const randomizeDefense = Math.floor(Math.random()*this.state.pokemons[0].stats[2].base_stat);
    const randomizeAttack = Math.floor(Math.random()*this.state.pokemons[1].stats[1].base_stat);
    const hit = randomizeAttack-randomizeDefense > 0 ? randomizeAttack - randomizeDefense : 0;
    const HP = this.state.poke1HP - hit > 0 ? (this.state.poke1HP - hit) : 0;
    let messages = [this.state.pokemons[0].name + (hit ? ` took ${hit} in damage` : ` successfully blocked hit`)];
    if (HP === 0) {
      messages.push(`${this.state.pokemons[0].name} passed out`);
      messages.push(`${this.state.pokemons[1].name} won`);
      this.sendWinner(this.state.pokemons[1].name);
    }

    this.setState({poke1HP: HP});
    this.setState({battleStatus: messages});

  }

  sendWinner = (winner) => {
    this.props.winner(winner);
  }
}
